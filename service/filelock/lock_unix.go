...
//
// If the *Filelock has a shared lock (RLock),
...
func (f *Filelock) lock(locked *bool, flag int) error {
	f.m.Lock()
	defer f.m.Unlock()
...
// If we get the lock, we also set the *Filelock instance as being exclusive-locked.
...
// the function will return false instead of waiting for the lock.
// If we get the lock, we also set the *Filelock instance as being share-locked.
func (f *Filelock) TryRLock() (bool, error) {
	return f.try(&f.r, unix.LOCK_SH)
}

func (f *Filelock) try(locked *bool, flag int) (bool, error) {
	f.m.Lock()
	defer f.m.Unlock()
...
func (f *Filelock) reopenFDOnError(err error) (bool, error) {
