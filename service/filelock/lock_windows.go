...
func (f *Filelock) lock(locked *bool, flag uint32) error {
	f.m.Lock()
	defer f.m.Unlock()
...
// the function will return false instead of waiting for the lock.
// If we get the lock, we also set the *Filelock instance as being shared-locked.
func (f *Filelock) TryRLock() (bool, error) {
	return f.try(&f.r, winLockfileSharedLock)
}

func (f *Filelock) try(locked *bool, flag uint32) (bool, error) {
	f.m.Lock()
	defer f.m.Unlock()
